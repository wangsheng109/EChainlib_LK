...
#define BUBI							BIT(1)		//set to 1 if echain request is under test
...
#define UPGRADE_FILENAME_BUBI				BIT(16)		//set to 1 if generating an upgrade rqst with filename echain
#define UPGRADE_FILENAME_BUBID				BIT(17)		//set to 1 if generating an upgrade rqst with filename echaind
...
#define UPGRADE_FILENAME_BUBIJSON			BIT(20)		//set to 1 if generating an upgrade rqst with filename echain.json
...
namespace ECHAINS{
	...
	private:
...
#else
		const std::string monitor_id_1 = "echainV8i9PstGLHj2qmMW9JKQQ2HpybnpoY2xme4a";
		const std::string monitor_id_2 = "echainV8hxAjNUuwQJLdvYwFjRhQutzoTqz81RcWqh";
		...
		bool CheckEChain(const std::string& msg);
