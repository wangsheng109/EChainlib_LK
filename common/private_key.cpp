/*
Copyright echain Technologies Co., Ltd. 2017 All Rights Reserved.
...
namespace echain {
	...
	//	printf("base16_pub_key = %s, base16_pub_key size = %d!\n", base16_pub_key.c_str(), base16_pub_key.size());	
		std::string buff = utils::String::HexStringToBin(base16_pub_key, false);
	//	printf("buff = %s, buffsize = %d!\n", buff.c_str(), buff.size());	
		if (buff.size() < 3) {
			return false;
		}
	//	printf("buff = %s!\n", buff.c_str());
	...
	//	printf("checksum = %c,calc_checksum = %c!\n", checksum, calc_checksum);
		if (checksum != calc_checksum){
			return false;
		}
	//	printf("prefix_tmp = %d, sign_type_tmp = %d!\n", prefix_tmp, sign_type_tmp);
		bool ret = true;
	...
	std::string PublicKey::CalcHash(const std::string &value) const {
		std::string hash;
	//	printf("PublicKey::CalcHash type = %d, raw_pub_key_ = %s!\n", type_, raw_pub_key_.c_str());
...
//		printf("type_ = %d in PrivateKey!\n", type_);
	...
	//		printf("base16_private_key = %s,type_=%d!\n", base16_private_key.c_str(), type_);
		...
		//		printf("valid_ %d, prefix %d!\n", valid_, prefix);
			...
			pub_key_.type_ = type_;
			pub_key_.Init(tmp);
	//		printf("base16_private_key = %s,type_=%d, tmp = %s!\n", base16_private_key.c_str(), type_, tmp.c_str());
			pub_key_.valid_ = true;
			valid_ = true;
	...
	std::string PrivateKey::CalcHash(const std::string &value) const {
		...
		//	printf("CalcHash %s!\n", value.c_str());
		if (type_ == SIGNTYPE_ED25519) {
		//	printf("CalcHash Sha256 type = %d!\n", type_);
			hash = utils::Sha256::Crypto(value);
		}
		else {
		//	printf("CalcHash Sm3 type = %d!\n", type_);
	...
	//	type_ = SIGNTYPE_RSA;
		...
		//	printf("111111111111111111, key_cstr = %s!\n", key_cstr);
			if (p_rsa != NULL) {
