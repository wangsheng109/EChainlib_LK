...
{
	mysql_init(&g_mysql);//初始化MYSQL标识符，用于连接
	//设置自动重连
	char szValue[1] = {'1'};
...
//	NBDLog("%s!\n", rec_accouts.context.c_str());	
	if (!reader.parse(revMessage.context.c_str(), rep))  // reader将Json字符串解析到rep，rep将包含Json里所有子元素	
	{
...
//	NBDLog("%s!\n", rec_accouts.context.c_str());	
	...
	if (rep.isMember("ledger_manager"))
	{
		...
		iTxsPerTime = iTxNums*3/iBlocks;  //一天只能算8个小时，24小时的1/3
		if(iTxsPerTime <= 1)
...
int ParseMetaInfoFromTransactionHash(std::string transactionHash, std::string &transactionInfo)
{
	...
	if (!reader.parse(rec.context.c_str(), rep))  // reader将Json字符串解析到rep2，rep2将包含Json里所有子元素	
	{
...
int getPrivateKeyFromUserRegisterDB(std::string srcAddress, std::string &prvKey)
{
	...
	if(iResult)
	{
		printf("查询数据库失败%s,sqlcmd is %s\n",mysql_error(&g_mysql),SQLCmd);
		NBDLog("查询数据库失败%s,sqlcmd is %s\n",mysql_error(&g_mysql),SQLCmd);
		g_dbmutex.Unlock();
		return -1;
	}
	else 
	{
		g_res = mysql_store_result(&g_mysql);//返回查询的全部结果集
...
//	printf("EChain_TransferPayment submitTransaction response is \n%s\n", rec.context.c_str());	
	...
	//	NBDLog("Payment submitTransaction failed, result = %d!\n", iResult);
		...
		else if((iResult == 100)||(iResult == 104)) //余额不足
		{
			if(pHexData != NULL)
			{
				free(pHexData); 
...
int EChain_BusinessDataUpdate(std::string contractAddress, int updateType, int businessType, std::string preTranxHash, std::string dataHash, std::string dataPath, std::string descInfo, std::string &transactionHash)
{
	...
	Json::Value meteJson;
	if (!reader.parse(transactionInfo.c_str(), meteJson))  // reader将Json字符串解析到rep2，rep2将包含Json里所有子元素	
	{
...
输入参数：
          ...
          contractAddress:合约帐号
          businessType: 用户类型  
          transactionHash: 最近一次链上数据的交易Hash值
          dataHash: 数据摘要
	  conditionInfo: 数据查询条件（条件表达式）
输出参数：
	 dataResult: 数据计算结果
	 result:返回验证结果
