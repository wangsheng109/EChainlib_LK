...
protected:
	virtual void SetUp()
	{
		priv_key = new echain::PrivateKey(echain::ED25519SIG);
	}
...
TEST_F(PrivateKeyTest,PrivateKey2){
	echain::PrivateKey priv_key2(priv_key->GetBase58PrivateKey());
...
TEST_F(PrivateKeyTest, Verify){
	...
	//执行测试验证
	EXPECT_TRUE(false);
	EXPECT_TRUE(echain::PublicKey::Verify(data, signature, publicKey));
	EXPECT_FALSE(echain::PublicKey::Verify(data + "1", signature, publicKey));
	EXPECT_FALSE(echain::PublicKey::Verify(data, signature + "1", publicKey));
//	EXPECT_FALSE(echain::PublicKey::Verify(data, signature, publicKey + "1"));
	//EXPECT_FALSE(echain::PublicKey::Verify(data, signature,""));
	EXPECT_FALSE(echain::PublicKey::Verify(data, "", publicKey));
	EXPECT_FALSE(echain::PublicKey::Verify("", signature, publicKey));
	...
	publicKey2 += "a";
	EXPECT_FALSE(echain::PublicKey::Verify(data2, signature, publicKey));
	EXPECT_FALSE(echain::PublicKey::Verify(data, signature2, publicKey));
	EXPECT_FALSE(echain::PublicKey::Verify(data, signature, publicKey2));
}

//测试PublicKey类中方法
TEST_F(PrivateKeyTest, PublicKey){
	echain::PublicKey pub;
	EXPECT_TRUE(echain::PublicKey::IsAddressValid(priv_key->GetBase58Address()));
