...
namespace utils {
	...
	public:
		...
		explicit InetAddress(const boost::asio::ip::tcp::endpoint &endpoint);
		explicit InetAddress(const boost::asio::ip::udp::endpoint &endpoint);
	...
	public:
		...
		bool Close();
		bool AttachServiceIo(boost::asio::io_service *io);
	...
	protected:
		...
		boost::asio::io_service *io_service_ptr_;
	};
	...
	private:
		boost::asio::ip::tcp::socket *tcpsocket_ptr_;
	...
	class AsyncSocketAcceptor : public AsyncSocket {
		friend class AsyncIo;
	private:
		boost::asio::ip::tcp::acceptor *acceptor_ptr_;
	...
	private:
		boost::asio::ip::udp::socket *udpsocket_ptr_;
		char buffer_[utils::ETH_MAX_PACKET_SIZE];
	...
	class AsyncSocketSsl : public AsyncSocket {
		...
		friend class AsyncSocketAcceptor;
	public:
		AsyncSocketSsl(AsyncIo *asyncio_ptr, boost::asio::ssl::context& context);
	...
	public:
		...
		virtual bool OnVerifyCertificate(bool preverified, boost::asio::ssl::verify_context& ctx);
	...
	class NameResolver {
	private:
		boost::asio::ip::tcp::resolver *resolver_ptr_;
	public:
		NameResolver(boost::asio::io_service &async_io_lptr);
		~NameResolver();
